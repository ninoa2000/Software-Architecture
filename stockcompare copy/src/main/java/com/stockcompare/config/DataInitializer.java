package com.stockcompare.config;
...
public class DataInitializer {
    ...
    @Bean
    @Transactional
    public CommandLineRunner initData(StockRepository stockRepository, StockPriceRepository stockPriceRepository) {
        return args -> {
            // Check if data already exists by counting stocks
            long stockCount = stockRepository.count();
            logger.info("Found " + stockCount + " existing stocks in database");
            
            // Force reinitialize data for testing purposes
            if (stockCount > 0) {
                logger.info("Clearing existing stock data for fresh initialization");
